...
def init_worker(dictperuser, dicturlidx, separatedoutputdirpath):

    global dict_per_user, dict_url_idx, separated_output_dir_path

    dict_per_user = dictperuser
    dict_url_idx = dicturlidx
    separated_output_dir_path = separatedoutputdirpath
...
def separated_process(args=(-1, [])):
    ...
    print(f"worker {worker_id}: len(user_ids) = {len(user_ids)}")
    dict_data = {}
...
def main():
    ...
    with Pool(processes=n_multi, initializer=init_worker, initargs=(dict_per_user, dict_url_idx, separated_output_dir_path)) as pool:
